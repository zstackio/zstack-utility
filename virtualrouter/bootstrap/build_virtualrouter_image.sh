...
UBUNTU='Ubuntu'
VERSION='6'
WORKSPACE=/root/build_zstack_vr_image

MNT=`mktemp`
rm -f $MNT
mkdir -p $MNT

IMG_NAME=${WORKSPACE}/zstack-vr-`date +%Y%m%d`.img
IMG_SIZE=2500 #2.5G
...
install_ubuntu(){
    ...
    debootstrap --include=less,sudo,openssh-server,acpid,python-virtualenv,netcat,telnet,dnsmasq,dnsmasq-base,dnsmasq-utils,python-setuptools,gcc,autoconf,curl,gfortran,python-pip,python-simplejson,python-cherrypy,python-routes,python-paramiko,python-pyparsing,python-dev,python-apt,tcpdump,iputils-arping,iptables $UBUNTU_OPTS $UBUNTU_RELEASE $MNT $UBUNTU_REPO || call_exception "cannot install $UBUNTU_RELEASE into $IMG_NAME"
...
install_debian(){
    ...
    debootstrap --include=less,sudo,openssh-server,acpid,python-virtualenv,netcat,telnet,dnsmasq,dnsmasq-utils,python-setuptools,gcc,autoconf,curl,gfortran,python-pip,python-simplejson,python-cherrypy,python-routes,python-paramiko,python-pyparsing,python-dev,python-apt,acpi-support-base,tcpdump,iputils-arping,iptables $DEBOOTSTRAP_OPTS $DEBIAN_RELEASE $MNT $DEBIAN_REPO || call_exception "cannot install $DEBIAN_RELEASE into $IMG_NAME"
...
EOF
    ...
    echo_bold "copy appliance vm boot up service"
    /bin/cp -f $WORKSPACE/zstack-appliancevm-bootstrap.py $MNT/sbin/
    sed -i 's/^exit 0/python \/sbin\/zstack-appliancevm-bootstrap.py\nexit 0/' $MNT/etc/rc.local
}
...
config_rh_rc_local(){
    cp $WORKSPACE/zstack-appliancevm-bootstrap.py $MNT/sbin/
    cat >> $MNT/etc/rc.d/rc.local <<EOF
python /sbin/zstack-appliancevm-bootstrap.py
EOF
...
#For some fedora system, e.g. F18, might need following script.
#    #rc.local for systemd operations.
#    cat >> $MNT/etc/init.d/rc.local <<EOF
##!/bin/sh
##
## chkconfig: 345 99 01
## description: vr_boot_strap script.
##
#case $1 in
#        start)
#            echo_bold "your rc.local starting"
#            python /sbin/vrbootstrap.py
#            ;;
#        restart)
#            echo_bold "your rc.local stopping"
#            echo_bold "your rc.local starting"
#            ;;
#        status)
#            echo_bold "your rc.local running"
#            ;;
#        stop)
#            echo_bold "your rc.local stopping"
#            ;;
#esac
#EOF
#    chmod a+x $MNT/etc/init.d/rc.local
#    in_chroot chkconfig --add rc.local
...
rm -rf $IMG_NAME
mkdir -p $WORKSPACE 
/bin/cp -f `dirname $0`/zstack-appliancevm-bootstrap.py $WORKSPACE
