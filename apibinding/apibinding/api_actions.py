...
class ResumeSchedulerAction(inventory.APIResumeSchedulerMsg):
    def __init__(self):
        super(ResumeSchedulerAction, self).__init__()
        self.sessionUuid = None
        self.out = None
    def run(self):
        if not self.sessionUuid:
            raise Exception('sessionUuid of action[ResumeSchedulerAction] cannot be None')
        evt = api.async_call(self, self.sessionUuid)
        self.out = evt
        return self.out

class PauseSchedulerAction(inventory.APIPauseSchedulerMsg):
    def __init__(self):
        super(PauseSchedulerAction, self).__init__()
        self.sessionUuid = None
        self.out = None
    def run(self):
        if not self.sessionUuid:
            raise Exception('sessionUuid of action[PauseSchedulerAction] cannot be None')
        evt = api.async_call(self, self.sessionUuid)
        self.out = evt
        return self.out
...
class QueryKeystoreAction(inventory.APIQueryKeystoreMsg):
    def __init__(self):
        super(QueryKeystoreAction, self).__init__()
        self.sessionUuid = None
        self.reply = None
        self.out = None
    def run(self):
        if not self.sessionUuid:
            raise Exception('sessionUuid of action[QueryKeystoreAction] cannot be None')
        reply = api.sync_call(self, self.sessionUuid)
        self.reply = reply
        self.out = reply.inventories
        return self.out

class DeleteKeystoreAction(inventory.APIDeleteKeystoreMsg):
    def __init__(self):
        super(DeleteKeystoreAction, self).__init__()
        self.sessionUuid = None
        self.out = None
    def run(self):
        if not self.sessionUuid:
            raise Exception('sessionUuid of action[DeleteKeystoreAction] cannot be None')
        evt = api.async_call(self, self.sessionUuid)
        self.out = evt
        return self.out

class CreateKeystoreAction(inventory.APICreateKeystoreMsg):
    def __init__(self):
        super(CreateKeystoreAction, self).__init__()
        self.sessionUuid = None
        self.out = None
    def run(self):
        if not self.sessionUuid:
            raise Exception('sessionUuid of action[CreateKeystoreAction] cannot be None')
        evt = api.async_call(self, self.sessionUuid)
        self.out = evt
        return self.out
