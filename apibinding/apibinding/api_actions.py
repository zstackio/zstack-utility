...
class GetCurrentTimeMillisAction(inventory.APIGetCurrentTimeMillisMsg):
    def __init__(self):
        super(GetCurrentTimeMillisAction, self).__init__()
        self.sessionUuid = None
        self.out = None
    def run(self):
        if not self.sessionUuid:
            raise Exception('sessionUuid of action[GetCurrentTimeMillisAction] cannot be None')
        evt = api.async_call(self, self.sessionUuid)
        self.out = evt
        return self.out
