...
class SyncVolumeActualSizeAction(inventory.APISyncVolumeActualSizeMsg):
    def __init__(self):
        super(SyncVolumeActualSizeAction, self).__init__()
        self.sessionUuid = None
        self.out = None
    def run(self):
        if not self.sessionUuid:
            raise Exception('sessionUuid of action[SyncVolumeActualSizeAction] cannot be None')
        evt = api.async_call(self, self.sessionUuid)
        self.out = evt
        return self.out
