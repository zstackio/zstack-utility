...
class Api(object):
    ...
    def login_as_admin(self):
        ...
        if not reply.success: raise ApiError("Cannot login as admin because %s" % self._error_code_to_string(reply.error))
    ...
    def log_out(self, session_uuid):
        ...
        (name, reply) = self.sync_call(apicmd)
        if not reply.success: logger.warn('Logout session[uuid:%s] failed because' % (session_uuid, self._error_code_to_string(reply.error)))
    ...
    def async_call_wait_for_complete(self, apicmd, exception_on_error=True, interval=500, fail_soon=False):
        ...
        if not timeout: timeout = 1800000
        ...
        if curr >= timeout: raise ApiError('API call[%s] timeout after %dms' % (apicmd.FULL_NAME, curr))

        logger.debug("async call[url: %s, response: %s] after %dms" % (self.api_url, rsp.result, curr))
        reply = jsonobject.loads(rsp.result)
        (name, event) = (reply.__dict__.items()[0])
        if exception_on_error and not event.success: raise ApiError('API call[%s] failed because %s' % (name, self._error_code_to_string(event.error)))
        return (name, event)
...
def async_call(apicmd, session_uuid):
    ...
    if not event.success: raise ApiError("Async call: [%s] meets error: %s." % (apicmd.__class__.__name__, error_code_to_string(reply.error)))
...
def sync_call(apicmd, session_uuid):
    ...
    if not reply.success: raise ApiError("Sync call: [%s] meets error: %s." % (apicmd.__class__.__name__, error_code_to_string(reply.error)))
