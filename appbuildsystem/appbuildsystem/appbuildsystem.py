...
class AppBuildSystemAgent(object):
    ...
    @replyerror
    def create(self, req):
        def _read_image_info(srcDir):
            imageMeta = srcDir + "/application-image-meta.json"
            if not os.path.exists(imageMeta):
                raise Exception("cannot find image meta file: %s" % imageMeta)
            fd = open(imageMeta, 'r')
            ctx = fd.read()
            fd.close()
            return ctx
        ...
        rsp.imageInfos = _read_image_info(cmd.srcPath)
    ...
    def export(self, req):
        ...
        def _tar_export(srcDir, dstDir, ctx):
            ...
            dstPath = "%s/%s.tar" % (dstDir, basename)
