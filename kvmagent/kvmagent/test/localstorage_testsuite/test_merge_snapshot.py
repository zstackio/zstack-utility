...
class TestLocalStoragePlugin(TestCase):
    ...
    def test_merge_snapshot(self):
        ...
        rsp = localstorage_utils.merge_snapshot(
            snapshotInstallPath="/local_ps/test/test.qcow2",
            workspaceInstallPath="/local_ps/test/test/test.qcow2",
            storagePath="/local_ps"
        )
        self.assertEqual(True, os.path.exists("/local_ps/test/test/test.qcow2"), "[check] cannot find rootvolume in host")
