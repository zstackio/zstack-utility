...
class TestSharedBlockPlugin(TestCase, SharedBlockPluginTestStub):
    ...
    def test_sharedblock_get_volume_size(self):
        ...
        rsp = sharedblock_utils.sharedblock_get_volume_size(
            installPath="sharedblock://{}/{}".format(vgUuid, volumeUuid),
            vgUuid=vgUuid
        )

        self.assertEqual(rsp.actualSize, str(16 * 1024**2), rsp.error)
        self.assertEqual(rsp.size, 1048576, rsp.error)