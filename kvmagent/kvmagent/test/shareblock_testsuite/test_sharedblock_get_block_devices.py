...
class TestSharedBlockPlugin(TestCase, SharedBlockPluginTestStub):
    ...
    def test_sharedblock_get_block_devices(self):
        ...
        rsp = sharedblock_utils.shareblock_get_block_devices(hostUuid=hostUuid, vgUuid=vgUuid)
        self.assertEqual(True, rsp.success, rsp.error)
        self.assertNotEqual(0, len(rsp.blockDevices))