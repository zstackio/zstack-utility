...
#       3 in order to save time.
    ...
    def delete_bits(self, req):
        ...
        self.do_delete_bits(cmd.path, discard=cmd.issueDiscards, zeroed=cmd.zeroed)

        rsp.totalCapacity, rsp.availableCapacity = lvm.get_vg_size(cmd.vgUuid)
        rsp.lunCapacities = lvm.get_lun_capacities_from_vg(cmd.vgUuid, self.vgs_path_and_wwid)
        return jsonobject.dumps(rsp)

    def do_delete_bits(self, path, discard=lvm.LvDiscardStrategy.NEVER, zeroed=False):
        ...
        if lvm.has_lv_tag(install_abs_path, IMAGE_TAG):
            ...
            lvm.delete_lv(install_abs_path, discard=discard, zeroed=zeroed)
