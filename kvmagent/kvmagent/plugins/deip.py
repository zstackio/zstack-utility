...
class Eip(object):
    ...
    def apply_eip(self, eip):
        ...
        def delete_ebtables_chain_if_exists(table, chain):
            ...
            RULE = bash_o(EBTABLES_CMD + " -t {{table}} -L PREROUTING --Lx 2>/dev/null | grep -- '-j {{chain}}'").strip()
            if RULE:
            ...
            RULE = bash_o(EBTABLES_CMD + " -t {{table}} -L POSTROUTING --Lx 2>/dev/null | grep -- '-j {{chain}}'").strip()
            if RULE:
                for line in RULE.splitlines():
                    line = line.strip()
                    if line:
