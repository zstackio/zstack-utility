...
BOOTPROTO={{ proto }}
{% if proto == "dhcp" -%}
# network config when BOOTPROTO=dhcp, do not edit.
#.IPADDR={{ ip_address }}
#.NETMASK={{ netmask }}
{% if gateway -%}
#.GATEWAY={{ gateway }}
    ...
    def _get_current_config(self, ports_config):
        ...
        for path in files_list:
            ...
            for line in content.split('\n'):
                ...
                if len(info) != 2:
                    continue
                if not info[0].startswith('#') or info[0].startswith('#.'):
...
class VmQgaSyncResponse(kvmagent.AgentResponse):
    def __init__(self):
        super(VmQgaSyncResponse, self).__init__()
        self.states = None
