    ...
    def installtdc(self, req):
        ...
        if cmd.version != tdcversion:
            ...
            if startCmd.return_code != 0:
                ...
                yum_cmd = "export YUM0=`awk '{print $3}' /etc/zstack-release`; yum --enablerepo=zstack-mn,qemu-kvm-ev-mn clean metadata"
                shell.call(yum_cmd)
                ...
                if e.return_code != 0:
                    ...
                    yum_cmd = "export YUM0=`awk '{print $3}' /etc/zstack-release`; yum --disablerepo=* --enablerepo=zstack-mn,qemu-kvm-ev-mn install -y kernel-3.10.0-693.11.1.el7.x86_64-vrbd-1.0-0.1.release1.alios7.x86_64"
                    shell.call(yum_cmd)
                e = shell.ShellCmd('rpm -qi tdc-unified-8.2.0.release.el5.x86_64')
                e(False)
                if e.return_code != 0:
                    yum_cmd = "export YUM0=`awk '{print $3}' /etc/zstack-release`; yum --disablerepo=* --enablerepo=zstack-mn,qemu-kvm-ev-mn install -y tdc-unified-8.2.0.release.el5.x86_64"
                    shell.call(yum_cmd)
