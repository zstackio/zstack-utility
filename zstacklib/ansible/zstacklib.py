...
def run_remote_command(command, host_post_info, return_status=False, return_output=False):
    ...
    if 'yum' in command:
        set_yum0 = "rpm -q zstack-release >/dev/null && releasever=`awk '{print $3}' /etc/zstack-release` || releasever=%s;export YUM0=$releasever; grep $releasever /etc/yum/vars/YUM0 || echo $releasever > /etc/yum/vars/YUM0;" % (host_post_info.releasever)
