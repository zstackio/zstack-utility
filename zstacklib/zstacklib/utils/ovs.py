...
class OvsVenv(object):
    ...
    def init(self, hugepage_size=DEFAULT_HUGEPAGE_SIZE, nr_hugepages=DEFAULT_NR_HUGEPAGES):
        ...
        self._fillVersionInfo()
        self._fillNicOffloadStatus()
        self._makeDirForOvs()

    def _fillVersionInfo(self):
        if not os.path.exists("/usr/bin/ovs-vsctl") \
           or not os.path.exists("/usr/sbin/ovs-vswitchd") \
           or not os.path.exists("/usr/sbin/ovsdb-server"):
            raise OvsError(
                "Check OpenvSwitch installation failed. Try 'yum install openvswitch' and reinstall it.")
...
class OvsBaseCtl(object):
    ...
    def getBondFromFile(self, bondName):
        try:
    ...
    def _getBondSlaves(self, bondName):
        ...
        # dpdk bond
        dpdkBond = self.getBondFromFile(bondName)
    ...
    def getBondType(self, name):
        ...
        bond = self.getBondFromFile(name)
        if bond:
...
class OvsDpdkCtl(OvsBaseCtl):
    ...
    def _initDpdk(self):
        ...
        if self.isDpdkReady:
            ...
            raise OvsError("ovs do not support dpdk.")
    ...
    def addDpdkBondToBr(self, bondName, bridgeName):
        bond = self.getBondFromFile(bondName)
