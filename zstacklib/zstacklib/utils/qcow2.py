...
def create_template_with_task_daemon(src, dst, task_spec, dstFormat='qcow2', opts=None, **daemonargs):
    ...
    with ConvertTaskDaemon(dst, task_spec):
        linux.create_template(src, dst, shell=t_shell, progress_output=p_file, opts=opts, dstFormat=dstFormat)
