...
class TestIpv6RouteRestore(unittest.TestCase):
    def test_restore_ipv6_route_drops_runtime_expiration(self):
        ...
        call.assert_called_once_with(
            'ip -6 route replace default via fe80::1 dev br_zsn1 '
            'proto ra metric 1024 hoplimit 64 pref medium'
        )

    def test_build_ipv6_route_preserves_stable_values(self):
        route = (
            'default via fe80::1 dev zsn1 proto ra metric 1024 '
            'expires 172sec src 2001:db8::10 mtu 1400 hoplimit 64 pref medium'
        )

        self.assertEqual(
            linux._build_ipv6_route(route, 'br_zsn1'),
            'default via fe80::1 dev br_zsn1 proto ra metric 1024 '
            'src 2001:db8::10 mtu 1400 hoplimit 64 pref medium'
        )
    ...
    def test_restore_ipv6_route_keeps_routes_without_expiration(self):
        ...
        call.assert_called_once_with(
            'ip -6 route replace 2001:db8:1::/64 dev br_zsn1 proto kernel'
        )

    def test_delete_ipv6_route_drops_runtime_expiration(self):
        route_info = {
            'ipv4_addresses': [],
            'ipv6_addresses': [],
            'routes': [],
            ...
            'routes6': [
                'default via fe80::1 dev zsn1 proto ra metric 1024 '
                'expires 172sec pref medium'
            ],
            'connected_routes6': [],
        }

        with patch.object(linux.shell, 'call') as call:
            linux._delete_dev_route('zsn1', route_info)

        call.assert_called_once_with(
            'ip -6 route del default via fe80::1 dev zsn1 '
            'proto ra metric 1024 pref medium', exception=True
        )
